...
import UIKit
   ...
   public class RaisePlaceholder: TextField {
    ...
    required public init?(coder aDecoder: NSCoder) {
        ...
        if self.isSecureTextEntry{
            button.setImage(#imageLiteral(resourceName: "eye_off"), for: .normal)
            button.imageEdgeInsets = UIEdgeInsetsMake(2, 2, 2, 2)
            button.frame = CGRect(x: CGFloat(self.frame.size.width - 25), y: CGFloat(13), width: CGFloat(24), height: CGFloat(20))
    ...
    fileprivate func createPlaceholderLabel() {
        ...
        if !self.label.isDescendant(of: self) {
            ...
            label.text = ""
            label.font = Font.setFont(name: AvenirNextLTPro_Medium, size: self.placeholderLabelFontSize)//UIFont.systemFont(ofSize: self.placeholderLabelFontSize)
    ...
    public func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        if let placeholderLabel = self.placeholderLabel, self.text == "" {
            ...
            UIView.animate(withDuration: animationDuration, animations: {
                ...
                placeholderLabel.font = Font.setFont(name: AvenirNextLTPro_Medium, size: self.placeholderLabelFontSize)//UIFont.systemFont(ofSize: self.placeholderLabelFontSize)
    ...
    @IBAction func refresh(_ sender: Any) {
        if self.isSecureTextEntry{
            button.setImage(#imageLiteral(resourceName: "eye_on"), for: .normal)
